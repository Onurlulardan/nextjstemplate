import { Adapter, AdapterUser, AdapterAccount, AdapterSession, AdapterVerificationToken } from "next-auth/adapters";
...
export function NextAuthKnexAdapter(knex: Knex): Adapter {
  return {
    ...
    async updateSession(data) {
      const [session] = await knex("Session").where({ sessionToken: data.sessionToken }).update(data).returning("*");
      return session;
    },
